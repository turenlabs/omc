...
[oss-microcode-runtime.md](oss-microcode-runtime.md).
...
cargo run -p omc-cli --bin omc -- --project-dir /tmp/omc-demo npm view left-pad version -j
cargo run -p omc-cli --bin omc -- --project-dir /tmp/omc-demo npm -j view left-pad version
...
- `omc npm` compatibility commands for common `install`, `link` / `ln`, `update` / `up` /
  ...
  dependency tree instead of raw OMC lock entries and respects `--depth`,
...
- `omc pip` compatibility commands for common `install`, `lock`, `uninstall`, `freeze`,
  ...
  with pip-style aligned columns, compact JSON, editable-location and verbose JSON metadata, and freeze output,
  `-r`, index URL,
