...
requirements.txt  PyPI requirements, direct wheel URLs/paths, hashes, extras, local editable/direct paths, -r includes, -c constraints, markers, simple indexes, find-links wheelhouses
...
pyproject.toml    PEP 621 project dependencies, selected optional groups, dependency-groups, direct wheel URL/path dependencies, local path deps
...
install Python console_scripts from wheels and local path packages into .omc/python/bin
...
- `requirements.txt` ingestion with hashes, line continuations, extras,
  ...
  editable/direct paths, and common Python environment markers
...
- `pyproject.toml` PEP 621 dependency ingestion with `omc install --extra`
  ...
  with nested `include-group` support, direct wheel URLs/paths, and local path
  dependencies
...
- npm bin links, including linked workspace/local package bins, and Python
  console script shims from wheels and local path packages
